...
func reportPageHandler(store app.APIStore) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		artifactURL := ""
		if job.ScanType != app.ScanTypeSingle && jobAllowsPluginArtifact(job) {
...
func bubbleLabelFontSize(text string, diameter int, maxPx, minPx float64) float64 {
	...
	available := float64(diameter) * 1.15
	estimated := available / (float64(chars) * 0.54)
	if estimated > maxPx {
		return maxPx
	}
	if estimated < minPx {
...
func sampleTimelinePoints(points []analyzer.TimelinePoint, limit int) []analyzer.TimelinePoint {
	...
	lastSource := -1
	for i := 0; i < limit; i++ {
		source := int(float64(i) * float64(len(points)-1) / float64(limit-1))
