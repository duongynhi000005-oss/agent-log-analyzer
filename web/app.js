  ...
  try {
    ...
    if (session.job_id && session.report_path) {
      paidStatus.textContent =
        `paid token expires ${new Date(session.expires_at).toLocaleTimeString()} - review the benchmark-backed local scan command before running it`;
...
function renderReport(report) {
  ...
  document.querySelector("#waste").textContent =
    `${report.estimated_waste_pct.low}-${report.estimated_waste_pct.high}% avoidable token spend detected in this session. Our repeated Agent Analyzer benchmark cut published API-rate cost by 23.986%; on $5,000/month of comparable Claude Sonnet coding usage, that scales to about $1,199/month.`;
  ...
  switch (finding?.id) {
    case "repeated_file_reads":
      return {
        ...
        plugin: "The plugin packages repeated-path patterns from this scan into scoped retrieval hygiene prompts.",
      };
    case "tool_output_bloat":
      return {
        ...
        plugin: "The plugin can recommend output-budget rules and context-safe command habits for your setup.",
      };
    case "retry_loop":
...
function recommendationPurpose(rec) {
  ...
  if (signals.includes("no_usage_visibility")) {
    return "Usage visibility tools make token burn and cache behavior visible. We classify them as measurement tools, not direct token savers.";
  }
  return "This recommendation is matched from a benchmark-narrowed allowlist against deterministic waste signals in the report.";
}
...
function recommendationPluginPitch(rec) {
  ...
  return "The plugin packages benchmark-backed recommendations and avoids one-off manual setup guesses.";
}
